...
export default function CrawlerPanel() {
  ...
  const { data, isLoading } = useQuery({ queryKey: ['crawler-pages'], queryFn: crawlerAPI.getPages, refetchInterval: 5000 });
  const pages: any[] = Array.isArray(data) ? data : [];
  const crawlMutation = useMutation({
    mutationFn: (u: string) => crawlerAPI.start(u),
  ...
  return (
    <div className="space-y-6">
      <div className="flex gap-3">
        ...
        <button onClick={() => crawlMutation.mutate(url)} disabled={!url || crawlMutation.isPending}
      ...
      <div className="rounded-xl border border-gray-200 overflow-hidden">
        <table className="w-full text-sm">
          <thead className="bg-gray-50 text-gray-500 text-xs uppercase tracking-wide">
            <tr><th className="text-left px-4 py-3">URL</th><th className="text-left px-4 py-3">Status</th><th className="text-left px-4 py-3">Chunks</th><th className="px-4 py-3"></th></tr>
          </thead>
          <tbody className="divide-y divide-gray-100">
            {isLoading && <tr><td colSpan={4} className="text-center py-8 text-gray-400">Loading...</td></tr>}
            {!isLoading && pages.length === 0 && <tr><td colSpan={4} className="text-center py-8 text-gray-400">No pages crawled yet</td></tr>}
            {pages.map((page: any) => (
              <tr key={page.id} className="hover:bg-gray-50">
                <td className="px-4 py-3 max-w-xs truncate text-blue-600">{page.url}</td>
                <td className="px-4 py-3">
                  <Badge variant={page.status === 'processed' ? 'success' : page.status === 'failed' ? 'danger' : 'warning'}>{page.status}</Badge>
                </td>
                <td className="px-4 py-3 text-gray-600">{page.chunks_count ?? '—'}</td>
                <td className="px-4 py-3 text-right">
                  <button onClick={() => deleteMutation.mutate(page.id)} className="p-1.5 text-gray-400 hover:text-red-600 hover:bg-red-50 rounded-lg transition-colors"><Trash2 size={14} /></button>
                </td>
              </tr>
            ))}
          </tbody>
